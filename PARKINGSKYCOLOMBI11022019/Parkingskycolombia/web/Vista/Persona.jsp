...
<input title="Se necesita llenar este campo" type="text" name="IdentificacionUsuario" value="<%if (request.getAttribute("C_ide") != null) {%>${C_ide}<%}%>" required><br>
               
 
                <Label>Nombres</Label><br>
<input type="text" name="NombreUsuario" value="<%if (request.getAttribute("C_nom") != null) {%>${C_nom}<%}%>" required><br>
               
                    <Label>Apellidos</Label><br>
<input type="text" name="ApellidoUsuario" value="<%if (request.getAttribute("C_ape") != null) {%>${C_ape}<%}%>" required><br>
                    
             
         <Label>Telefono</Label><br>
<input type="text" name="TelefonoUsuario" value="<%if (request.getAttribute("C_tel") != null) {%>${C_tel}<%}%>" required><br>
               
         <Label>Correo </Label><br>
<input type="text" name="CorreoUsuario" value="<%if (request.getAttribute("C_cor") != null) {%>${C_cor}<%}%>" required><br>
               
                       <Label>Usuario Red</Label><br>
     <input type="text" name="id_UsuarioPersona" value="<%if (request.getAttribute("C_usu") != null) {%>${C_usu}<%}%>" required><br>
                         <br>
                          <Label>Password</Label><br>
<input type="password" name="PasswordPersona" value="<%if (request.getAttribute("C_pas") != null) {%>${C_pas}<%}%>" required><br>
                         <br>
 
<Label>Rol Usuario</Label><br>
     <input type="text" name="RolUsuario" value="<%if (request.getAttribute("C_rol") != null) {%>${C_rol}<%}%>" required><br>
                      ...
                      <Label>Tipo Documento</Label><br>
                       <center> <select name="selettdoc">
                            ...
                            <% //Bloque de Codigo para consultar el Tipo de Documento
                                ...
                                cn.rt = DPersona.consultarDescripcionTD();
                                while (cn.rt.next()) {
